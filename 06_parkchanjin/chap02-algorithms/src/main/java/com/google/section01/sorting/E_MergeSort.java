...
/* 병합 정렬
*  - 분할 정복(Divide and Conquer) 방식으로 배열
*    배열을 반씩 나누어(1~2개 요소가 남을 때까지) 정렬한 후 병합하는 방식
...
public class E_MergeSort {
  ...
  * */
  ...
  private static void mergeSort(int[] arr, int[] temp, int left, int right){
    ...
    // <-> 분할된 배열의 크기가 1 이하인 경우 수행 X
    //     --> if문 수행 종료 == 분할된 배열이 종료 완료되었다
    if(left < right){
  ...
  private static void merge (int[] arr, int[] temp, int left, int mid, int right) {
    ...
    // -> 이 과정에서 정렬이 됨
    // 두 부분 배열에 모두 남은 요소가 있을때만 반복
    while (leftIndex <= mid && rightIndex <= right){

      // 임시 배열의 leftIndex 가 가리키는 값이
      // leftIndex가 가리키는 값이 rightIndex가 가리키는 값 보다
      // 같거나 작은 경우
      if(temp[leftIndex] <= temp[rightIndex]){
        arr[current++] = temp[leftIndex++];
      }
      // 임시 배열의 rightIndex 가 가리키는 값이
      // leftIndex 가리키는 값보다 작은경우
      else {
