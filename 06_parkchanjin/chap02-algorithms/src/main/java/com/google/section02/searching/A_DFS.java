package com.google.section02.searching;
...
*  - 시작 노드에서 출발하여 한 방향으로 갈 수 있는 만큼 깊이 탐색 한 후
...
public class A_DFS {
  ...
  * */
  ...
  static int count; // 감염된 컴퓨터 수(1번 제외)

  public static Integer solution(String input) throws IOException{

    BufferedReader br = new BufferedReader(new BufferedReader(new StringReader(input)));
    ...
    // 노드 방문 여부를 저장할 배열 생성
    // - 방문하면 true로 변환 ( visit[1] = true ---> 1번 노드 방문한 적 있음 )
  ...
  private static void dfsRecursive(int start) {
    ...
    for(int i = 1; i < node; i++) {
      ...
      /* start 노드와 연결 되어있고 map[start][i] == 1
      *  아직 방문한적이 없는 이웃 노드 찾기 !visit[i]
      * */
      if(map[start][i] == 1 && !visit[i]){
        count++;

        /* 3단계 : 찾은 이웃 노드를 방문해서
        *          그 노드와 연결된 이웃을 또 찾는 과정을 반복(재귀호출)
        *          메서드가 종료되면 호출한 곳(이전 노드)로 돌아옴(== 백트래킹)
        * */
