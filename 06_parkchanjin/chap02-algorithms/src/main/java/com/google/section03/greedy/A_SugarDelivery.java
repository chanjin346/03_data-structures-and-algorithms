...
public class A_SugarDelivery {
  ...
  /**
   * 3kg과 5kg 봉지로 가져갈 수 있는 최소 봉지 수
   *
   * @param n 배달해야 할 설망의 무게
  ...
  public static int solution(int n) {
    ...
    while (true) {
      if (n % 5 == 0) return count + n / 5; // 5kg로 해결 가능한 경우
      else if (n < 0) return -1; // 불가능
      else if (n == 0) return count; // 3kg로만 처리가능한 경우
